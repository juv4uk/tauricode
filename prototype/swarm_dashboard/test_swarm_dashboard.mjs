...
import { MOCK_SWARM_TOPOLOGY, MOCK_TASK_STATS, CANONICAL_DERIVATIONS, PHONEME_DATA_REGISTRY } from "./fixtures.js"
import { getSwarmTopology, queryDerivationTrace, streamPhonemeVector } from "./tauri_ipc.js"
...
async function runTests() {
  ...
  assert(
    MOCK_TASK_STATS.totalCompleted >= 271,
    `Total completed tasks (${MOCK_TASK_STATS.totalCompleted}) exceeds target of 271+`,
  )
  assert(
    MOCK_TASK_STATS.completionRatePct >= 95.0,
    `Mesh completion rate (${MOCK_TASK_STATS.completionRatePct}%) is above 95%`,
  )

  const sumTasksByNodes = Object.values(MOCK_TASK_STATS.nodeDistribution).reduce((a, b) => a + b, 0)
  assert(
    sumTasksByNodes === MOCK_TASK_STATS.totalCompleted,
    `Node task distribution sum (${sumTasksByNodes}) matches total (${MOCK_TASK_STATS.totalCompleted})`,
  )
  ...
  assert(
    traceBhavati.derivation_id === "drv:canonical:bhavati-v0.1",
    "query_derivation_trace returns canonical bhavati",
  )
