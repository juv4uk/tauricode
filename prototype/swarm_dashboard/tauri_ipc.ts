...
import { SwarmMeshTopology, DerivationTrace, PhonemeVectorData, SwarmNode, DerivationState } from "./types"
import { MOCK_SWARM_TOPOLOGY, CANONICAL_DERIVATIONS, PHONEME_DATA_REGISTRY } from "./fixtures"
...
export async function streamPhonemeVector(phonemeOrCode: string | number): Promise<PhonemeVectorData> {
  ...
  const matched =
    ...
    Object.values(PHONEME_DATA_REGISTRY).find(
      (p) => p.slp1 === key || p.phoneme === key || p.upc8Hex.toLowerCase() === key,
    ) ||
